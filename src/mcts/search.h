...
class Search {
 public:
  ...
  // Beam search methods
  void MakeBeamSearchMove();
  std::vector<Node*> GetBeamSearchCandidates(Node* node);
  int GetDynamicBeamWidth(Node* node) const;
 ...
 private:
  ...
  // Beam search methods
  void BeamSearchIteration();
  void UpdateBeamSelectionMultiplier();
  float ComputeBeamSelectionScore(const EdgeAndNode& edge) const;
